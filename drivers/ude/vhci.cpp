...
{
        PAGED_CODE();
        enum { MIN_PORTS = 1, DEF_PORTS = 30, MAX_PORTS = 256 };

        usb2_ports = DEF_PORTS;
        usb3_ports = DEF_PORTS;
        ...
        struct {
                ...
                { L"NumberOfUsb20Ports", usb2_ports },
                { L"NumberOfUsb30Ports", usb3_ports }
        };
        ...
        for (auto& [name, value]: params) {
                ...
                if (ULONG val{}; auto err = WdfRegistryQueryULong(key.get(), &value_name, &val)) {
                        ...
                        value = min(MAX_PORTS, max(MIN_PORTS, val));
                }
        }
}
